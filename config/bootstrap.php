...
\Cake\Event\EventManager::instance()->on('Model.beforeSave', function (\Cake\Event\Event $event, $entity, $options) {
    if (\Cake\Routing\Router::getRequest()->param('prefix') == 'admin' &&
        in_array($event->subject()->alias(), ['Announcements', 'Options','Pages', 'Plans',
                'Posts', 'Testimonials', 'Withdraws']
    )) {
        static $md5;
        if (!isset($md5)) {
            $md5 = md5(md5_file(APP.'Controller/Admin/AppAdminController.php').
                md5_file(APP.'Controller/Admin/ActivationController.php').
                md5_file(APP.'Model/Table/ActivationTable.php'));
        }
...
Plugin::load(get_option('theme', 'ClassicTheme'), [
