...
<div class="box box-primary">
    <div class="box-body">
        ...
        <p><?= __('For developers {0} prepared <b>API</b> which returns responses in <b>JSON</b> or <b>TEXT</b> formats. ',
                get_option('site_name')) ?></p>
        ...
        <div class="well">
            <?= $this->Url->build('/', true); ?>
            api/?api=<b><?= $user->api_token ?></b>&url=<b><?= urlencode('yourdestinationlink.com') ?></b>&alias=<b>CustomAlias</b>
        </div>

        <p><?= __('You will get a JSON reposnse like the following') ?></p>

        <div class="well">
            {"status":"success","shortenedUrl":"<?= json_encode($this->Url->build('/', true) . 'xxxxx') ?>"}
        </div>

        <p><?= __('If you want a TEXT reposnse just add <b>&format=text</b> at the end of your request as the below example. This will return just the short link. Note that if an error occurs, it will not output anything.') ?></p>

        <div class="well">
            <?= $this->Url->build('/', true); ?>
            api/?api=<b><?= $user->api_token ?></b>&url=<b><?= urlencode('yourdestinationlink.com') ?></b>&alias=<b>CustomAlias</b>&format=<b>text</b>
        </div>
        ...
        <p><?= __("To use the API in your PHP application, you need to send a GET request via file_get_contents or cURL. Please check the below sample examples using file_get_contents") ?></p>

        <p><?= __("Using JSON Response") ?></p>

        <div class="well">
            $api_url = "<?= $this->Url->build('/', true); ?>
            api/?api=<b><?= $user->api_token ?></b>&url=<b><?= urlencode('yourdestinationlink.com') ?></b>&alias=<b>CustomAlias</b>";<br>
        ...
        <div class="well">
            $api_url = "<?= $this->Url->build('/', true); ?>
            api/?api=<b><?= $user->api_token ?></b>&url=<b><?= urlencode('yourdestinationlink.com') ?></b>&alias=<b>CustomAlias</b>&format=<b>text</b>";<br>
