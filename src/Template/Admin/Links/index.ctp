...
<?php foreach ($links as $link): ?>
    ...
    <div class="box box-solid">
        <div class="box-body">
            <h4><a href="<?= $short_url ?>" target="_blank"><span
                            class="glyphicon glyphicon-link"></span> <?= h($title) ?></a></h4>
            <p class="text-muted">
                <small><i class="fa fa-bar-chart"></i> <a href="<?= $short_url ?>/info"
                                                          target="_blank"><?= __('Stats') ?></a> - <i
                            class="fa fa-user"></i> <?= $this->Html->link($link->user->username,
                        ['controller' => 'Users', 'action' => 'view', $link->user_id]); ?> - <i
                            class="fa fa-calendar"></i> <?= display_date_timezone($link->created); ?> - <a
                            target="_blank" href="<?= $link->url ?>"><?= strtoupper(parse_url($link->url,
                            PHP_URL_HOST)); ?></a></small>
            </p>
            <div class="row">
                <div class="col-sm-6">
                ...
                <div class="col-sm-6">
                    <div class="text-right">
                        ...
                        <?= $this->Form->postLink(__('Deactivate'), ['action' => 'deactivate', $link->alias],
