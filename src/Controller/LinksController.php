...
class LinksController extends FrontController
{
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $this->Auth->allow(['shorten', 'st', 'full', 'api', 'view', 'go', 'stats', 'popad']);
    }
    ...
    public function view($alias = null)
    {
        ...
        // No Ads
        if ($plan_direct || $link->ad_type == 0) {
            $this->updateLinkHits($link);
            $this->addNormalStatisticEntry($link, $link->ad_type, [
                'ci' => 0,
                'cui' => 0,
                'cii' => 0,
        ...
        if ($plan_disable_captcha ||
            ...
            $campaign_item = $this->getCampaignItem($link->ad_type, $traffic_source, $country);
            ...
            $this->set('pop_ad', $this->dataEncrypt($pop_ad));
    ...
    public function popad()
    {
        ...
        if ($this->request->is('post')) {
            $pop_ad = $this->dataDecrypt($this->request->data['pop_ad']);
    ...
    protected function dataEncrypt($value)
    {
        $key = Security::salt();
    ...
    protected function dataDecrypt($value)
    {
        $key = Security::salt();
    ...
    protected function calcEarnings($data, $link, $ad_type)
    {
         ...
         */
        /*
          if (!isset($_SERVER["HTTP_CF_CONNECTING_IP"]) && $this->isProxy()) {
          // Update link hits
          $this->updateLinkHits($link);
          $this->addNormalStatisticEntry($link, $ad_type, $data, $cookie['ip'], 5);
          $content = [
          'status' => 'success',
        ...
        if ($publisher_user_earnings && !empty($user_update->referred_by)) {
            $user_referred_by = $this->Links->Users->find()
                ->where(['id' => $user_update->referred_by, 'status' => 1])
                ->first();
