...
class AppController extends Controller
{
    ...
    public function beforeFilter(Event $event)
    {
        ...
        if ($this->forceSSL()) {
            return $this->redirect('https://' . env('SERVER_NAME') . $this->request->here, 301);
        }

        // Check if you are on the main domain
        if ($this->redirectMainDomain()) {
            $protocol = (empty($_SERVER['HTTPS']) || $_SERVER['HTTPS'] === "off") ? "http://" : "https://";
            return $this->redirect($protocol . get_option('main_domain') . $this->request->here, 301);
        }
    ...
    protected function forceSSL()
    {
        if ((bool)get_option('ssl_enable', false)) {
            ...
            if (!(in_array($controller, ['Links', 'Invoices']) &&
                in_array($action, ['st', 'api', 'view', 'go', 'popad', 'ipn'])
            )
            ) {
